...
static void testCreateWriteReadBufferManagerLRU(int n, int m);
...
main (void) 
{
  ...
  testName = "";
    ...
    testCreateWriteReadBufferManagerLRU(20, 8);
...
static void testCreateWriteReadStorageManager(int n)
{
    ...
    TEST_CHECK(createPageFile("testbuffer_lru.bin"));
    TEST_CHECK(openPageFile("testbuffer_lru.bin", &file_handle));
...
static void testCreateWriteReadBufferManagerLRU(int n, int m)
{
    ...
    char **pages1;
    char **pages2;
    testName = "Create, Write and Read LRU Buffer Manager";
    SM_FileHandle file_handle;
    ...
    TEST_CHECK(openPageFile("testbuffer_lru.bin", &file_handle));
    TEST_CHECK(initBufferPool(&bm, "testbuffer_lru.bin", m, RS_LRU, NULL));
    ...
    // Scenario 4 - test LRU replacement
    for (i = 0; i < m / 2; i++) {
        TEST_CHECK(pinPage(&bm, &mempages[i], (PageNumber) i));
    ...
    for (i = 0; i < m / 2; i++) {
        ...
        ASSERT_TRUE(1 == fix_counts[i], "SC4 check remaining counts");
    }
