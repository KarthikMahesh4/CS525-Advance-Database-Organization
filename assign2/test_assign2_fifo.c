...
static void testCreateWriteReadBufferManagerFIFO(int n, int m);
...
main (void) 
{
  ...
  testName = "";
    ...
    testCreateWriteReadBufferManagerFIFO(20, 8);
...
static void testCreateWriteReadStorageManager(int n)
{
    ...
    TEST_CHECK(createPageFile("testbuffer_fifo.bin"));
    TEST_CHECK(openPageFile("testbuffer_fifo.bin", &file_handle));
...
static void testCreateWriteReadBufferManagerFIFO(int n, int m)
{
    ...
    char **pages1;
    char **pages2;
    testName = "Create, Write and Read FIFO Buffer Manager";
    SM_FileHandle file_handle;
    ...
    TEST_CHECK(openPageFile("testbuffer_fifo.bin", &file_handle));
    TEST_CHECK(initBufferPool(&bm, "testbuffer_fifo.bin", m, RS_FIFO, NULL));
    ...
    // Scenario 4 - test FIFO replacement
    for (i = 0; i < m / 2; i++) {
        TEST_CHECK(pinPage(&bm, &mempages[i], (PageNumber) i));
    ...
    for (i = m; i < m + m / 2; i++) {
        ASSERT_TRUE(i == frame_contents[(i - m)], 
                "SC4 check remaining frames");
        ASSERT_TRUE(false == dirty_flags[(i - m)],
                "SC4 check remaining flags");
        ASSERT_TRUE(1 == fix_counts[(i - m)], 
