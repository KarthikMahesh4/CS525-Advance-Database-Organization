...
int RS_fifo_init(RS_fifo_t *rs, const char *pageFile, int numPages)
{
    ...
    rs->num_pages = 0;
    rs->frames = (fifo_frame_t*) malloc(sizeof(fifo_frame_t) * numPages);
    rs->next_ticket = 1;
    rs->read_io = 0;
    rs->write_io = 0;
    
    /* initialize all the frames with NO_PAGE page number, zeros for data, false
     * dirty flag, zero fix count and zero ticket */
        ...
        const PageNumber pageNum)
{
    ...
    // search for the frame and if it exists, just return it
    for (i = 0; i < rs->max_num_pages; i++) {
    ...
    if (rs->num_pages < rs->max_num_pages ) {
        ...
        for (i = 0; i < rs->max_num_pages; i++) {
            if (rs->frames[i].page_num == NO_PAGE) {
                ...
                rs->frames[i].dirty = false;
                rs->frames[i].fixcount++;
                rs->frames[i].ticket = rs->next_ticket;
                rs->next_ticket++;
        ...
        min_ticket= ULONG_MAX;
        k = -1;

        for (i = 0; i < rs->max_num_pages; i++) {
            if (rs->frames[i].page_num != NO_PAGE 
                    && rs->frames[i].fixcount == 0
                    && rs->frames[i].ticket < min_ticket) {
                min_ticket = rs->frames[i].ticket;
                k = i;
            }
        }
        ...
        rs->frames[k].ticket = rs->next_ticket;
        rs->next_ticket++;
